...
新增“标的发掘”模块：在账户追踪工作区中读取富途自选股，按当前趋势策略筛选最接近建仓条件的标的，并给出入场参考、止损参考、止盈参考和首仓金额建议。当前功能仍保持只读分析，不自动下单、不解锁交易。

### 本次新增

- 新增 `src/trend_option_backtest/screening.py`，把标的筛选和评分逻辑从 UI 中拆出：
  - 按均线趋势、价格突破、成交量确认、行业共振、未过热 5 个维度评分。
  - 输出综合评分、接近程度、转入场条件、提前关注点、入场/止损/止盈参考价和首仓参考金额。
  - 支持把符合条件和未命中标的拆分展示，优先显示高分候选。
- 富途 provider 新增自选股读取能力：
  - 支持读取自选股分组。
  - 支持按分组读取标的，并兼容 `code`、`stock_code`、`symbol` 等返回字段。
  - 股票代码继续统一为应用内富途格式，如 `US.AMD`、`HK.00700`。
- 账户追踪工作区新增 `🔍 标的发掘` Tab：
  - 提供“读取自选股”和“刷新筛选”入口。
  - 支持选择自选股分组。
  - 符合入场条件的标的用卡片展示，包含信号雷达、信号标签和价格参考。
  - 其他关注标的按自选分组折叠展示，便于观察最接近触发的候选。
  - 符合条件的标的可一键准备加入回测标的池。

### Bug 修复与体验改进

- 修复账户追踪模式下 `WorkspaceContext` 缺少 `futu_host`、`futu_port`、`futu_cache_dir` 导致页面报错的问题。
- 修复顶部品牌标题在浏览器中被裁切的问题：提高主容器顶部安全间距，并把品牌区改为更稳健的 CSS 类布局。
- 修复富途自选股读取频率限制体验：
  - 识别“频率太高 / 请求过于频繁 / rate limit”等错误。
  - 遇到富途限流后自动按 31 秒窗口重试。
  - 全量读取多个自选股分组时按“30 秒最多 10 次”的限制节流，避免一次性打爆 OpenD。
  - UI 层增加 30 秒重复点击保护和加载提示，避免用户误以为页面卡死。
- 修复标的发掘筛选里的 DataFrame 布尔判断问题，避免 pandas 对象进入 `or` 表达式导致异常。
- 优化标的发掘卡片布局，减少雷达图和价格框溢出风险。
...
- 顶部标题完整显示。
- 账户追踪工作区包含 `🔍 标的发掘` Tab。
- 富途自选股限流时会显示明确提示，并按接口限制节流重试。

### 下一步建议

1. 增加自选股读取结果的本地持久缓存，OpenD 限流或离线时可先使用最近一次成功结果。
2. 为标的发掘增加“只读预检”按钮，单独检查 OpenD 连接、自选股权限、分组数量和预计读取耗时。
3. 把标的发掘结果加入复盘档案，记录每次筛选时哪些标的接近入场、后来是否触发。
